...
where (bp.rank_update in ('S','A','B','C') or (bp.rank1 in ('S','A','B','C') and bp.creation >= '2023-11-01') ) -- if F rank and modifiled over 3months not need
...
-- Past prospect F 3months or less https://docs.google.com/spreadsheets/d/1HoPxR88w3o-_vGvFEpqxmIVztBLf4XU6vJm5x2NZmfg/edit#gid=1964911175
select bp.modified `Timestamp`, concat('http://13.250.153.252:8000/app/sme_bo_and_plan/', name) `Edit`,
	-- sme.dept `DEPT`, sme.sec_branch `SECT`, sme.unit_no `Unit_no`, sme.unit `Unit`, bp.staff_no `Staff No`, sme.staff_name `Staff Name`, 
	sme.dept `DEPT`, 
	sme.sec_branch `SECT`, 
...
-- Past prospect F 6months or less https://docs.google.com/spreadsheets/d/1HoPxR88w3o-_vGvFEpqxmIVztBLf4XU6vJm5x2NZmfg/edit#gid=1118523635
select bp.modified `Timestamp`, concat('http://13.250.153.252:8000/app/sme_bo_and_plan/', name) `Edit`,
	-- sme.dept `DEPT`, sme.sec_branch `SECT`, sme.unit_no `Unit_no`, sme.unit `Unit`, bp.staff_no `Staff No`, sme.staff_name `Staff Name`, 
	sme.dept `DEPT`, 
	sme.sec_branch `SECT`, 
...
-- ___________________________________________________________________ 00 ___________________________________________________________________
select bp.modified `Timestamp`, concat('http://13.250.153.252:8000/app/sme_bo_and_plan/', name) `Edit`,
	-- sme.dept `DEPT`, sme.sec_branch `SECT`, sme.unit_no `Unit_no`, sme.unit `Unit`, bp.staff_no `Staff No`, sme.staff_name `Staff Name`, 
	case when bp.callcenter_of_sales is null or bp.callcenter_of_sales = '' then sme.dept else smec.dept end `DEPT`, 
	case when bp.callcenter_of_sales is null or bp.callcenter_of_sales = '' then sme.sec_branch else smec.sec_branch end `SECT`, 
...
from tabSME_BO_and_Plan bp left join sme_org sme on (case when locate(' ', bp.staff_no) = 0 then bp.staff_no else left(bp.staff_no, locate(' ', bp.staff_no)-1) end = sme.staff_no)
left join sme_org smec on (regexp_replace(bp.callcenter_of_sales  , '[^[:digit:]]', '') = smec.staff_no)
where date_format(bp.creation, '%Y-%m-%d') >= date(now()) and bp.rank_update in ('S', 'A', 'B', 'C', 'F') 
order by case when rank_update in ('S','A','B','C') then 1 else 0 end desc, sme.id asc;
